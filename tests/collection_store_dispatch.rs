...
use torii::{
    ...
    services::collection_store::{CollectionStoreRepos, CollectionStoreResolver, ResolvedCollectionStore},
};
...
fn collection_store_resolves_managed_and_linked_by_collection_id() -> Result<()> {
    ...
    let managed_env_count: i64 = db.block_on(async {
        sqlx::query_scalar("SELECT COUNT(*) FROM environments WHERE collection_id = ?")
            .bind(managed.id.to_string())
            .fetch_one(db.pool())
            .await
    })?;
